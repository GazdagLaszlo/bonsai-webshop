<template>        
    <div class="my-6">
        ...
        <div class="is-flex is-flex-direction-row">
            <div class="column box is-three-fifths p-6 mb-0 mr-4" :style="{backgroundColor: COLORS.box}">
                    ...
                    <li v-for="item in cartStore.items" :key="item.id">
                        <div class="columns cart-item mb-5 is-flex is-flex-direction-row">
                            ...
                            <div class="column is-four-fifths">
                                <div class="is-flex is-flex-direction-row is-justify-content-space-between" :style="{cursor: 'pointer'}">
                                    <p class="has-text-weight-bold">{{item.name}}</p>
                                    <figure class="image is-32x32" @click="cartStore.removeFromCart(item.id)">
                                ...
                                <div class="is-flex is-flex-direction-row is-align-items-space-between">
                                    <div class="column is-flex is-flex-direction-row">
                                        <button @click="cartStore.decreaseItemQuantity(item.id)" class="button quantity-setter-button is-size-7">
                                            -
                                        </button>
                                        <p class="has-text-weight-semibold mx-4 py-1">{{item.quantity}} db</p>
                                        <button @click="cartStore.increaseItemQuantity(item.id)" class="button quantity-setter-button is-size-7">
                                            +
                                        </button>
                                    </div>
